 ...
 * @ingroup compressionThreads
 * @description
 *     Dynamic polling function for Data Compression instances that supports
 *     both epoll-based event notification and traditional busy-wait polling.
 *     The function dynamically switches between polling modes based on the
 *     instance's current response mode setting, allowing runtime optimization
 *     of polling behavior.
 *
 * @context
 *     This function runs in a polling thread and continues until the DC
 *     service is stopped (dc_service_started_g == CPA_FALSE).
 *
 * @assumptions
 *     - instanceHandle is valid and initialized
 *     - SUPPORTED_FEAT_EPOLL is defined and epoll is available
 *     - dcPollingThreadsInterval_g is set to appropriate nanoseconds value
 *
 * @sideEffects
 *     - May temporarily change instance response mode for epoll setup
 *     - Sets error_flag_g on critical errors
 *     - Allocates memory for epoll events (cleaned up before return)
 *
 * @blocking
 *     Yes - blocks on epoll_wait() or nanosleep() calls
 *
 * @reentrant
 *     No
 *
 * @threadSafe
 *     No - should be called from a single polling thread per instance
 *
 * @param[in] instanceHandle  DC instance handle to poll
 *
 * @return void
 ******************************************************************************/
void sampleCodeDcDynamicPoll(CpaInstanceHandle instanceHandle_in);
